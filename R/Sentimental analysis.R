GetSentiment <- function(product_code,page_range){
  ...
  sentiment <- tokens %>% dplyr::inner_join(tidytext::get_sentiments("bing")) %>%
    ...
    dplyr::mutate(sentiment = 3*negative-2*positive) #need to consider the formula again!
  a <- sentiment [[3]]
  if (a<0){print("This product recieved good reviews")} else {print("BE CAREFUL! This product recieved BAD reviews")}
